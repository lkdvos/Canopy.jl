...
function main()
    ...
    open(md, "w") do io
        ...
        println(io, "machine-independent. `:fz2_u1_flat` is census-only (not in `BENCH_SPACES`):")
        println(io, "it is `fℤ₂ ⊠ U1Irrep` with the sector count pinned at 4, so it isolates the")
        println(io, "growing axis inside the same symmetry group as `:fz2_u1`.")
        println(io)
