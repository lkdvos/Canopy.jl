...
#
# on a cycle graph (1D PBC), via simple-update on top of belief-propagation
# messages. Sweeps μ at fixed t, computes ground-state energy per site E/L
# and filling ⟨n⟩, and compares against the exact single-particle solution
# at the same finite L.
#
# Run:  julia --project=examples examples/free_fermion_ring.jl
...
# Spinless tight-binding on a length-L PBC ring: ε(k) = -2t cos(k) - μ at
# k = 2π n / L for n = 0, …, L-1. Ground state fills every mode with
# ε(k) < 0; modes at ε(k) = 0 are unfilled (consistent convention at the
# band edge).
...
# so Σ_e h_e = H exactly. On a ring every vertex has degree 2.
