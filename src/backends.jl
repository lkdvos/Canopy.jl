...
Select the `Layout(k)` formulation of the vertex-batched BP message kernel
...
uses `inner`. See also [`PairwiseBackend`](@ref).
"""
...
Force the pairwise (default) BP message kernel, with `inner` as the underlying
TensorOperations backend.

This is what [`BlockedBackend`](@ref) is differentially tested against: it is
explicit rather than relying on the default, so both kernels can be run on
